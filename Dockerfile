...
WORKDIR /src
COPY ["AzureFunctionEventHubTriggerMissingInfo.csproj", "."]
RUN dotnet restore "./AzureFunctionEventHubTriggerMissingInfo.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "AzureFunctionEventHubTriggerMissingInfo.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "AzureFunctionEventHubTriggerMissingInfo.csproj" -c Release -o /app/publish /p:UseAppHost=false
